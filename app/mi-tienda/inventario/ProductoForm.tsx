...
export default function ProductoForm({ initialData = null, onSave, onCancel }: ProductoFormProps) {
  ...
  useEffect(() => {
    ...
    console.log("🏷️ Cargando marcas para usuario:", usuario.uid);
    obtenerMarcasUsuario(usuario.uid).then(marcasData => {
      console.log("🏷️ Marcas cargadas:", marcasData);
      setMarcas(marcasData);
    }).catch(err => {
      console.error("❌ Error cargando marcas:", err);
    });
    obtenerBodegasUsuario(usuario.uid).then(bodegasData => {
      console.log("🏭 Bodegas cargadas:", bodegasData);
    ...
    try {
      ...
      const imagenesProcesadas = await Promise.all(imagenes.map(async (img: string | File, idx: number) => {
        if (typeof img === "string") {
          ...
          const path = `productos/${usuario?.uid || 'default'}/${nombreArchivo}`;
          try {
